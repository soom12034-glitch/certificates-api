...
public partial class MaintenanceReceiptWindow : Window
{
    ...
    sealed class ReceiptViewModel : ViewModelBase
    {
        const string ReceiptPrinterNameKey = "MaintenanceReceiptPrinterName";
        const string ReceiptOffsetYKey = "MaintenanceReceiptOffsetY";
        const string ReceiptOffsetXKey = "MaintenanceReceiptOffsetX";
        const string ReceiptTopMarginKey = "MaintenanceReceiptTopMargin";
        const string ReceiptBottomMarginKey = "MaintenanceReceiptBottomMargin";
        const string ReceiptIsLetterheadKey = "MaintenanceReceiptIsLetterhead";
        const string ReceiptFrameColorKey = "MaintenanceReceiptFrameColor";
        ...
        public ReceiptViewModel(MaintenanceViewModel.MaintenanceReceiptData data)
        {
            _data = data;
        ...
        void Print()
        {
            try
            {
                ...
                dialog.PrintDocument(paginator, "Maintenance Receipt");
            }
            catch (Exception ex)
            {
        ...
        Brush GetFrameBrush()
        {
            return SelectedFrameColor switch
            {
                ...
                _ => new SolidColorBrush(Color.FromRgb(80, 80, 80)) // رمادي داكن
            };
        }
        ...
        FixedDocument BuildDocument()
        {
            ...
            const double pageMarginLeftRight = 40; // قللنا الهامش ليأخذ البرواز راحته
            ...
            // رسم خطوط الهوامش الوهمية للمعاينة إذا كان مروساً
            if (IsLetterhead)
            {
            ...
            // Viewbox to guarantee it fits on one page vertically
            var viewbox = new Viewbox
            {
                Width = a4Width - (pageMarginLeftRight * 2), // Viewbox width matches available width
            ...
            // البرواز الخارجي الذي يحيط بكل المحتوى
            var outerBorder = new Border
            {
                BorderBrush = GetFrameBrush(),
                BorderThickness = new Thickness(SelectedFrameColor == "بدون برواز" ? 0 : 2),
                CornerRadius = new CornerRadius(12), // حواف ناعمة جداً
                Padding = new Thickness(24, 24, 24, 24), // مساحة داخلية للبرواز
                Background = Brushes.White
            };

            var root = new StackPanel
            {
                Width = a4Width - (pageMarginLeftRight * 2) - 48 // الخصم من أجل المسافات الداخلية للبرواز
            };
            
            outerBorder.Child = root;
            viewbox.Child = outerBorder;

            // Optional Dynamic Company Header
            ...
            var dateLine = new TextBlock
            {
                ...
                Text = $"التاريخ: {_data.ReceivedDate:yyyy-MM-dd}",
                Margin = new Thickness(0, 0, 0, 4)
            };
            var receiptNumberLine = new TextBlock
            {
                FlowDirection = FlowDirection.RightToLeft,
                FontFamily = new FontFamily("Tahoma"),
                FontSize = 14,
            ...
            metaStack.Children.Add(dateLine);
            metaStack.Children.Add(receiptNumberLine);
            ...
            AddRow(customerGrid, 1, "رقم الهاتف", Safe(_data.CustomerPhone), false, true);
            ...
            foreach (var device in _data.Devices)
            {
                var deviceLabel = new TextBlock
                {
                    ...
                    Text = _data.Devices.Count > 1 ? $"جهاز رقم {deviceCount}:" : "بيانات الجهاز:",
                    Margin = new Thickness(0, 0, 0, 10),
                    Width = tableWidth,
                    HorizontalAlignment = HorizontalAlignment.Center
                };
                ...
                AddRow(deviceGrid, 0, "نوع الجهاز", Safe(device.DeviceType), true);
                AddRow(deviceGrid, 1, "الموديل", Safe(device.Model));
                AddRow(deviceGrid, 2, "رقم السيريال", Safe(device.SerialNumber));
                AddRow(deviceGrid, 3, "وصف العطل", Safe(device.Complaint));
                AddRow(deviceGrid, 4, "المرفقات", Safe(device.Accessories), false, true);
            ...
            // Declaration Box
            var declarationBorder = new Border
            {
            ...
            var declarationText = new TextBlock
            {
                FlowDirection = FlowDirection.RightToLeft,
                FontFamily = new FontFamily("Tahoma"),
                FontSize = 14,
                ...
                Text = "أقر أنا الموقع أدناه باستلام الجهاز بالبيانات المذكورة أعلاه، وأتحمل المسؤولية الكاملة عن صحة البيانات المدخلة والمرفقات المسلمة مع الجهاز."
            };
            declarationBorder.Child = declarationText;
            root.Children.Add(declarationBorder);

            // Signatures
            var sig = new Grid { Width = tableWidth, HorizontalAlignment = HorizontalAlignment.Center, Margin = new Thickness(0,0,0,20) };
        ...
        static void AddRow(Grid grid, int rowIndex, string label, string value, bool isFirst = false, bool isLast = false)
        {
            ...
            valueCell.Child = valueText;
            Grid.SetRow(valueCell, rowIndex);
            Grid.SetColumn(valueCell, 1);
            grid.Children.Add(valueCell);
        }

        static UIElement MakeSignatureBlock(string title, int col)
        {
            var stack = new StackPanel { FlowDirection = FlowDirection.RightToLeft, Margin = col == 0 ? new Thickness(0, 0, 20, 0) : new Thickness(20, 0, 0, 0) };
