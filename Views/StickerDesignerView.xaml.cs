...
public partial class StickerDesignerView : UserControl
{
    ...
    void RedrawGrid()
    {
        ...
        var w = Math.Max(0.0, StickerCanvas.ActualWidth);
        var h = Math.Max(0.0, StickerCanvas.ActualHeight);
        if (w <= 0 || h <= 0) return;
    ...
    void StickerThumb_OnDragDelta(object sender, DragDeltaEventArgs e)
    {
        ...
        var canvasW = Math.Max(0, StickerCanvas.ActualWidth);
        var canvasH = Math.Max(0, StickerCanvas.ActualHeight);
        ...
        var proposedX = item.X + e.HorizontalChange;
        var proposedY = item.Y + e.VerticalChange;
        
        // Apply snapping if enabled
        if (DataContext is StickerDesignerViewModel vm && vm.SnapToGrid)
        {
             const double MmToPx = 1.0 / 0.264583;
    ...
    void StickerResizeThumb_OnDragDelta(object sender, DragDeltaEventArgs e)
    {
        ...
        var proposedW = item.Width + e.HorizontalChange;
        var proposedH = item.Height + e.VerticalChange;
